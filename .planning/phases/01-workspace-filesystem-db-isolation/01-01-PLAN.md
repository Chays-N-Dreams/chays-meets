...
must_haves:
  truths:
    - "Workspace data types are defined and usable by downstream plans"
    - "Filesystem operations can create a workspace directory with all required subfolders and files"
    - "Registry can be loaded, saved, and rebuilt from directory scan"
    - "Two separate migration SQL sets exist for workspace-scoped and global-scoped tables"
  artifacts:
