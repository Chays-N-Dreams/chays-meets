...
1. `pub async fn init(app_data_dir: PathBuf) -> Result<Self, String>`:
   ...
   - **IMPORTANT: Do NOT auto-create a Default workspace here. Do NOT auto-switch to last_active. The init() method only initializes the manager infrastructure (directories, global DB, registry). All migration detection, Default workspace creation, and workspace switching decisions are handled by `database/setup.rs::initialize_workspace_manager()` (Plan 03) AFTER init() returns.** This separation ensures migration detection can inspect the registry state before any workspaces are created.
...
4. Verify that `init()` does NOT create a Default workspace and does NOT call `switch_workspace()` -- it returns with `active_db: None`.
  </verify>
  <done>
WorkspaceManager compiles with: init(), active_pool(), global_pool(), switch_workspace(), create_workspace(), list_workspaces(), active_workspace_id(), workspaces_root(), close_active_workspace(). DatabaseManager has a `from_pool()` constructor. The init() method returns with no active workspace -- migration detection and workspace creation are deferred to setup.rs. The workspace module is fully self-contained and ready for integration.
  </done>
</task>
...
6. init() returns with active_db = None (no auto-creation, no auto-switch)
</verification>

<success_criteria>
- WorkspaceManager compiles and exports all required methods
- Pool lifecycle is correct: init opens global only, switch_workspace swaps active, close_active_workspace cleans up
- Registry is updated on create/switch operations
- Only one workspace pool is ever open at a time
- init() is pure infrastructure -- no workspace creation or switching logic
